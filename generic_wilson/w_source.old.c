...
static int invalid_ape_links = 1;
...
/* Set flag requiring remaking APE links.  Then free them */
void invalidate_ape_links(void){
  invalid_ape_links = 1;
  if(ape_links == NULL)return;
  destroy_G(ape_links);
  ape_links = NULL;
}
...
static void ape_smear_3D(Real staple_weight, int iters){
  int space_only = 1;
  ...
  /* Do not remake if flag is set */
  if(invalid_ape_links == 0)return;
...
int w_source_field(wilson_vector *src, wilson_quark_source *wqs)
{
  ...
  if(source_type == COMPLEX_FIELD_FILE){
    ...
    /* First construct space-like APE smeared links in "ape_links" */
    ape_smear_3D(wqs->staple_weight, wqs->ape_iter);

    /* Then do covariant Gaussian smearing on wqs->wv_src */
	...
	FORALLSITES(i,s){
      ...
      } else { /* FAT_COVARIANT_GAUSSIAN_DERIV2_B */
...
wilson_prop_field w_sink_op(wilson_quark_source *wqs, wilson_prop_field src )
{
  ...
  /* Various wave-function-based smearings requiring convolution */
	...
	for(spin=0;spin<4;spin++){
	  /* Take 12 derivative */
	  /* wv_src <- src_color_spin */
	  copy_wv_from_wp(wv_src, src, color, spin);
      ...
      /* dst <- D_dir1 src */
      cov_deriv_wp(dst, src, ape_links, wqs->dir1, wqs->disp,
		   wqs->weights);
    }

    /* Then do covariant Gaussian smearing on all t slices of dst */
...
int get_w_quark_source(FILE *fp, int prompt, wilson_quark_source *wqs){
  ...
  IF_OK {
    if ( source_type == COVARIANT_GAUSSIAN ){
      ...
      /* Parameters for spatial APE smearing */
      IF_OK status += get_f(stdin, prompt, "staple_weight", &staple_weight);
      IF_OK status += get_i(stdin, prompt, "ape_iter", &ape_iter);
  ...
  wqs->r0            = source_r0;
  wqs->staple_weight = staple_weight;
  wqs->t0            = source_loc[3];
  wqs->x0            = source_loc[0];
  wqs->y0            = source_loc[1];
  wqs->z0            = source_loc[2];
