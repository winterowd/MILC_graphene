...
/* Modifications ...
   ...
   8/21/06 UNFINISHED: for multimass inversions C.D. */
...
#define restrict rstrict /* C-90 T3D cludge */
  ...
  while( readin(prompt) == 0)
	...
	savelat_p = save_lattice( saveflag, savefile, stringLFN );
      ...
      /* Loop over kappas to open propagator files */
      for(k=0;k<num_kap;k++){
	...
	for(spinindex=0;spinindex<n_spins;spinindex++){
      ...
      /* Loop over kappas to close propagator file */
      for(k=0;k<num_kap;k++){
