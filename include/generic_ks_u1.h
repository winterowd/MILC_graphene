...
//*****// NEXT TO A BLOCK OF CODE MEANS THAT MAY HAVE TO MODIFY LATER BUT DO NOT KNOW AS OF YET
//4/11/2010
...
#endif
  complex *c_src;     /* Pointer for complex source field storage */
  complex *cv_src; /* Pointer for SU3 vector source field storage */
  int ksource;        /* Counter for a list of sources */
} ks_quark_source_u1; //****//
...
typedef struct {
  ...
  complex *U_link[4]; // original gauge matrices, stored as four fields
  complex *V_link[4]; // first iteration of fattening
  complex *Y_unitlink[4]; // unitary projection of V_link, U(3)
  complex *W_unitlink[4]; // special unitary projection of Y_link, SU(3)
  complex *XX_fatlink[MAX_NAIK][4];
  complex *XX_longlink[MAX_NAIK][4];
  complex *X_fatlink[4]; // these arrays are only pointers,
  complex *X_longlink[4];// they are not malloc'ed
...
} hisq_links_t_u1; //HAVE CHANGED ALL cases of "su3_matrix" to "complex" 4/11/2010
...
typedef struct {
  ...
  QOP_D3_FermionLinksAsqtad *qop_D_l;
#endif
} ferm_links_u1_t;//HAVE CHANGED ALL cases of "su3_matrix" to "complex" 5/11/2010
...
int ks_congrad_u1( field_offset src, field_offset dest, Real mass,
		int niter, int nrestart, Real rsqmin, int prec, 
		int parity, Real *rsq, ferm_links_u1_t *fn );

int ks_congrad_field_u1( complex *src, complex *dest, 
		      quark_invert_control *qic, Real mass,
		      ferm_links_u1_t *fn);

int ks_congrad_site_u1( field_offset src, field_offset dest, 
		     quark_invert_control *qic, Real mass,
		     ferm_links_u1_t *fn);
...
void dslash_fn_field_u1( complex *src, complex *dest, int parity,
			 ferm_links_u1_t *fn); //updated to U(1)
...
void clear_latvec_u1(field_offset v,int parity); /*Added 7/29/2010 for 
						   update_u1() */
...
void eo_fermion_force_oneterm_u1( Real eps, Real weight, field_offset x_off,
...
/* fermion_links_fn_u1. */
void init_ferm_links_u1(ferm_links_u1_t *fn);
void load_ferm_links_u1(ferm_links_u1_t *fn, ks_action_paths *ap);
void load_ferm_links_dmdu0_u1(ferm_links_u1_t *fn, ks_action_paths *ap);
void invalidate_all_ferm_links_u1(ferm_links_u1_t *fn);
void invalidate_fn_links_u1(ferm_links_u1_t *fn);
...
void load_longbacklinks_u1(ferm_links_u1_t *fn);
void load_fatbacklinks_u1(ferm_links_u1_t *fn);
void free_fn_links_u1(ferm_links_u1_t *fn);
...
void check_invert_field_u1_2( complex *src, complex *dest, Real mass,
			  Real tol, ferm_links_u1_t *fn);
