...
//  $Log: setup_old.c,v $
//  Revision 1.1.1.1  2013/04/29 23:19:53  winterow
...
//
//  MODIFIED FOR TESTING U1 matrix inversion 6/10/2010
*/
/* MIMD version 7 */
...
setup()
{
  ...
  init_ferm_links_u1(&fn_links_u1);
#ifdef DM_DU0
...
readin(int prompt)
{
  ...
  /* For right now just make a "cold lattice" (unit gauge links) 6/10/2010
  if( startflag == CONTINUE ){
    rephase( OFF );
  }
  if( startflag != CONTINUE )
...
#ifdef FN
  invalidate_all_ferm_links_u1(&fn_links_u1);
