...
/* Prototype...
   ...
   void gaugefix_u1(int gauge_dir,Real relax_boost,int max_gauge_iter,
	      Real gauge_fix_tol, 
   ...
   gaugefix_u1(TUP,(Real)1.5,500,(Real)1.0e-7,
   ...
   rephase( OFF );
   gaugefix_u1(YUP,(Real)1.8,500,(Real)2.0e-6,
   ...
   NOTE: if diffmat or sumvec are negative, gaugefix_u1 mallocs its own
   scratch space. */
...
static complex *diffmatp;               /* malloced diffmat pointer */
static complex *sumvecp;                /* malloced sumvec pointer */
...
/* hit should just be a phase for U(1) */
  ...
  FORSOMEPARITY(i,s,parity)
    {
      ...
      // U(1) left hit = Sum dir ( adj(link dir) + downlink dir )
      temp_left.real = sumvecp[i].real;
      temp_left.imag = sumvecp[i].imag;
      //normalize
      //norm = cabs(&temp_left);
      //CMULREAL(temp_left, 1./(norm), temp_left);
      //right hit = adj(left hit)
      CONJG(temp_left, temp_right);
      ...
      */
      ...
      /* Do U(1) hit on all upward links */

      //FORALLUPDIR(dir)
      CMUL(temp_left, s->link[TUP], s->link[TUP]);
      norm = cabs(&s->link[TUP]);
      CMULREAL(s->link[TUP], 1./(norm), s->link[TUP]);
      
      /* Do U(1) hit on all downward links */
      
      //FORALLUPDIR(dir)
      CMUL(temp_right, *(complex *)gen_pt[TUP][i], *(complex *)gen_pt[TUP][i]);
      norm = cabs((complex *)gen_pt[TUP][i]);
      CMULREAL(*(complex *)gen_pt[TUP][i], 1./(norm), *(complex *)gen_pt[TUP][i]);
...
double get_gauge_fix_action_u1(int gauge_dir,int parity)
{
  ...
  FORSOMEPARITY(i,s,parity)
    {
	  ...
	  gauge_fix_action += (double)m1->real;

	  //trace = trace_su3(m2); 
 	  //gauge_fix_action += (double)trace.real;
	  gauge_fix_action += (double)m2->real;
	  //}
    }
  ...
  /* Average is normalized to max of 1/2 on sites of one parity (KEEP??) */
  return(gauge_fix_action /((double)(2*nx*ny*nz*nt)));
} /* get_gauge_fix_action */

void gaugefixstep_u1(int gauge_dir,double *av_gauge_fix_action,Real relax_boost,
	      int nvector, field_offset vector_offset[], int vector_parity[],
	      int nantiherm, field_offset antiherm_offset[], 
	      int antiherm_parity[] )
{
  ...
  for(parity = ODD; parity <= EVEN; parity++)
    {
      ...
      //FORALLUPDIR(dir)
      //{
      mtag[TUP] = start_gather_site( F_OFFSET(link[TUP]), sizeof(complex),
				   OPP_DIR(TUP), parity, gen_pt[TUP] );
      //}
      ...
      gauge_fix_action = get_gauge_fix_action_u1(gauge_dir,parity);

      /* Do optimum gauge hit on various subspaces */

      /* Would this just be replaced by one hit with corresponding U(1)
	 element? 6/23/11 C.W. K*/
      ...
      *av_gauge_fix_action += gauge_fix_action;
      ...
      FORSOMEPARITY(i,s,parity)
	{
	  diffmatp[i].real = (*(complex *)(gen_pt[TUP][i])).real;
	  diffmatp[i].imag = (*(complex *)(gen_pt[TUP][i])).imag;
      ...
      mtag[TUP] = start_gather_field( diffmatp, sizeof(complex),
				    TUP, OPP_PAR(parity), gen_pt[TUP] );
	...
	s->link[TUP].real = (*(complex *)(gen_pt[TUP][i])).real;
	s->link[TUP].imag = (*(complex *)(gen_pt[TUP][i])).imag;
...
void gaugefixscratch_u1(void)
{
  diffmatp = (complex *)malloc(sizeof(complex)*sites_on_node);
  if(diffmatp == NULL)
    {
  ...
  sumvecp = (complex *)malloc(sizeof(complex)*sites_on_node);
  if(sumvecp == NULL)
    {
...
void gaugefix_combo_u1(int gauge_dir,Real relax_boost,int max_gauge_iter,
		    ...
		    int antiherm_parity[] )
{
  ...
  for (gauge_iter=0; gauge_iter < max_gauge_iter; gauge_iter++)
    {
      gaugefixstep_u1(gauge_dir,&current_av,relax_boost,
		   nvector, vector_offset, vector_parity,
		   nantiherm, antiherm_offset, antiherm_parity);
      ...
      if(gauge_iter != 0)
	{
	  ...
	  node0_printf("current_av = %e del_av = %e\n", current_av, del_av);
...
void gaugefix_u1(int gauge_dir,Real relax_boost,int max_gauge_iter,
	      Real gauge_fix_tol )
{
  gaugefix_combo_u1(gauge_dir, relax_boost, max_gauge_iter, gauge_fix_tol,
