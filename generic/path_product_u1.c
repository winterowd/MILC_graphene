...
//go through and update to complex arithmetic
void path_product_u1( const int *dir, const int length, complex *tempmat1) {
    register int i;
    register site *s;
    msg_tag *mtag0 = NULL;
    complex *tempmat2t, *tempmat3t;
    int j;
	...
	*/
    ...
    tempmat3t = (complex *)special_alloc( sites_on_node*sizeof(complex) );
    tempmat2t = (complex *)special_alloc( sites_on_node*sizeof(complex) );

    /* j=0 */
    if( GOES_FORWARDS(dir[0]) )  {
	mtag0 = start_gather_site( F_OFFSET(link[dir[0]]), sizeof(complex),
	...
	FORALLSITES(i,s){
	  if( i < loopend-FETCH_UP ){
	    ...
	    CONJG( s->link[OPP_DIR(dir[0])], tempmat1[i] );
	} END_LOOP
    }
	...
	if( j%2==1 ){
	    if( GOES_FORWARDS(dir[j]) ) {
	      if( GOES_FORWARDS(dir[j-1]) ){
	        ...
	        FORALLSITES(i,s){
		  if( i < loopend-FETCH_UP ){
		    prefetch_M( (complex *)(gen_pt[0][i+FETCH_UP]) );
		    prefetch_M(  &(tempmat2t[i+FETCH_UP]) );
		  }
		  CMUL( *(complex *)(gen_pt[0][i]), s->link[dir[j]],                                    tempmat2t[i] );
		} END_LOOP
		  ...
		  CMUL( tempmat1[i], s->link[dir[j]], tempmat2t[i] );
		} END_LOOP
	      }
	      mtag0 = start_gather_field( tempmat2t, sizeof(complex),
		OPP_DIR(dir[j]), EVENANDODD, gen_pt[0] );
		  ...
		  tempmat3t[i].real = (*(complex *)(gen_pt[0][i])).real;
		  tempmat3t[i].imag = (*(complex *)(gen_pt[0][i])).imag;
		} END_LOOP
	        cleanup_gather(mtag0);
	        mtag0 = start_gather_field( tempmat3t, sizeof(complex),
		  OPP_DIR(dir[j]), EVENANDODD, gen_pt[0] );
	      }
	      else{ /*last step was backwards */
	        mtag0 = start_gather_field( tempmat1, sizeof(complex),
		  OPP_DIR(dir[j]), EVENANDODD, gen_pt[0] );
	      }
	      wait_gather(mtag0);
	      FORALLSITES(i,s){
		...
		CMUL_J( *(complex *)(gen_pt[0][i]), s->link[OPP_DIR(dir[j])],
			tempmat2t[i] );
	      } END_LOOP
	      cleanup_gather(mtag0);
	  ...
	  if( GOES_FORWARDS(dir[j]) ) {
		...
		CMUL( *(complex *)(gen_pt[0][i]), s->link[dir[j]],
		      tempmat1[i] );
	      } END_LOOP
	      cleanup_gather(mtag0);
	    }
		...
		CMUL( tempmat2t[i], s->link[dir[j]], tempmat1[i] );
	      } END_LOOP
	    }
	    mtag0 = start_gather_field( tempmat1, sizeof(complex),
		OPP_DIR(dir[j]), EVENANDODD, gen_pt[0] );
	...
	wait_gather(mtag0);
	  FORALLSITES(i,s){
	    ...
	    tempmat1[i].real = (*(complex *)(gen_pt[0][i])).real;
	    tempmat1[i].imag = (*(complex *)(gen_pt[0][i])).imag;
	...
	FORALLSITES(i,s){
	  ...
	  tempmat1[i].real = tempmat2t[i].real;
	  tempmat1[i].imag = tempmat2t[i].imag;
	...
	FORALLSITES(i,s){
	  ...
	  tempmat1[i].real = tempmat3t[i].real;
	  tempmat1[i].imag = tempmat3t[i].imag;
