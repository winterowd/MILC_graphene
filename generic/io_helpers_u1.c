...
/* DT 8/97 
     ...
     that wants them.
     Only SciDAC I/O routines kept for the time being C.W. 11/2010 
*/
...
#ifndef NOLINKS
    d_plaquette_u1(&g_ssplaq,&g_stplaq);
    /* Remove these for now (not sure of their purpose with U(1) */
...
gauge_file *reload_lattice( int flag, char *filename){
    ...
    dtime = -dclock();
    //max_deviation = check_unitarity(); Need U(1) routine 11/2010
...
void funnylat_u1(void)  {
  //sets link matrices to funny matrices for debugging 
	...
	for(dir=XUP;dir<=TUP;dir++){
	  ...
	  //sit->link[dir].real = dir;
	  //sit->link[dir].real = 10*sit->x;
	  temp = (double)sqrt(2.)*(dir+2.);
	  //sit->link[dir].real = cos(temp);
	  //sit->link[dir].imag = dir;
	  //sit->link[dir].imag = 10*sit->z;
	  //sit->link[dir].imag = sin(temp);
	  //temp = (double)sqrt(2.)*dir
	  sit->link[dir].real = dir + sit->x;
	  sit->link[dir].imag = dir + sit->x;
	  //temp = 
	  //temp = (double)cabs(&(sit->link[dir]));
	  //CMULREAL(sit->link[dir], 1/temp, sit->link[dir]);
	}
    }
    //exp_links();
}//funnylat_u1 Added 7/29/2010
