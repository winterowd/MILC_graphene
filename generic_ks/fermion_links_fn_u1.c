...
#ifdef FN
  load_fatlinks_u1(fn, ap);
  load_longlinks_u1(fn, ap);
#endif

  fn->ap = ap;

#ifdef DBLSTORE_FN
  load_fatbacklinks_u1(fn);
  load_longbacklinks_u1(fn);
#endif
...
void
invalidate_all_ferm_links_u1(ferm_links_u1_t *fn)
{
  fn->valid = 0;
}
...
void
invalidate_fn_links_u1(ferm_links_u1_t *fn)
{
  invalidate_all_ferm_links_u1(fn);
}

void 
init_ferm_links_u1(ferm_links_u1_t *fn){
  fn->valid = 0;
  fn->fat = NULL;
  fn->lng = NULL;
  fn->fatback = NULL;
