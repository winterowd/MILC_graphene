...
ks_congrad_parity_u1( complex *t_src, complex *t_dest, 
  ...
  int special_started = 0; /* 1 if dslash_fn_field_special_u1 has been called */
  ...
  while(1) {
	...
	*/
	...
	/*FORALLMYSITES(i,s)
	  printf("t_dest[%d] = %f, %f\n", i, t_dest[i].real, 
	  t_dest[i].imag);*/
	dslash_fn_field_u1(t_dest, ttt, otherparity, fn);
	/*FORALLMYSITES(i,s)
	  printf("ttt[%d] = %f, %f\n", i, ttt[i].real, ttt[i].imag);*/
	dslash_fn_field_u1(ttt, ttt, parity, fn);
	/*FORALLMYSITES(i,s)
	  printf("ttt[%d] = %f, %f\n", i, ttt[i].real, ttt[i].imag);*/
    ...
    */
    ...
    if(special_started==0){
      dslash_fn_field_special_u1( cg_p, ttt, otherparity, tags2, 1, fn );
      dslash_fn_field_special_u1( ttt, ttt, parity, tags1, 1, fn);
      special_started=1;
    }
    else {
      /*FORALLSITES(i,s)
	printf("p[%d] = %f, %f\n", i, cg_p[i].real, cg_p[i].imag);*/
      dslash_fn_field_special_u1( cg_p, ttt, otherparity, tags2, 0, fn );
      /*FORALLSITES(i,s)
	printf("ttt[%d] = %f, %f\n", i, ttt[i].real, ttt[i].imag);*/
      dslash_fn_field_special_u1( ttt, ttt, parity, tags1, 0, fn);
      /*printf("D^2*p\n");
      FORALLSITES(i,s)
      printf("ttt[%d] = %f, %f\n", i, ttt[i].real, ttt[i].imag);*/
    }
    ...
    pkp = 0.0;
    temp_mul.real = 0.0; temp_conjg.real = 0.0; 
    temp_mul.imag = 0.0; temp_conjg.imag = 0.0;
#ifdef FEWSUMS
    c_tr=0.0; c_tt=0.0;
#endif
    FORMYPARITYDOMAIN(i,s,parity){
	...
	prefetch_VV( &ttt[i+FETCH_UP], &cg_p[i+FETCH_UP] );
      ...
      CMULREAL( cg_p[i], -1.0*msq_x4, temp_mul);
		      ...
		      ferm_links_u1_t *fn)
{
  ...
  if(parity == EVEN || parity == EVENANDODD){
    qic->parity = EVEN;
    iters += ks_congrad_parity_u1(src, dest, qic, mass, fn);
  }
		     ...
		     ferm_links_u1_t *fn)
{
  ...
  FORALLMYSITES(i,s){
    t_src[i]  = *((complex *)F_PT(s,src) );
    t_dest[i] = *((complex *)F_PT(s,dest));
  }

  if(parity == EVEN || parity == EVENANDODD){
    qic->parity = EVEN;
    iters += ks_congrad_parity_u1(t_src, t_dest, qic, mass, fn );
  }
  if(parity == ODD || parity == EVENANDODD){
    qic->parity = ODD;
    iters += ks_congrad_parity_u1(t_src, t_dest, qic, mass, fn );
...
int ks_congrad_u1( field_offset src, field_offset dest, Real mass,
  ...
  /* Solve the system */
  iters = ks_congrad_site_u1( src, dest, &qic, mass, fn );
...
//Site routines have been removed (partially)
